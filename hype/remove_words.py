...
from hype.hyla_utils import clean_str, loadWord2Vec
import sys
import argparse
import random
...
doc_content_list = []
f = open('text/corpus/' + dataset + '.txt', 'rb')
for line in f.readlines():
    doc_content_list.append(line.strip().decode('latin1'))
f.close()

with open('text/data/ind.train.ids', "w") as f:
    f.write('\n'.join([str(i) for i in train_ids]))
with open('text/data/ind.val.ids', "w") as f:
    f.write('\n'.join([str(i) for i in val_ids]))
with open('text/data/ind.test.ids', "w") as f:
...
f = open('text/corpus/' + dataset + '.clean.txt', 'w')
...
f = open('text/corpus/' + dataset + '.clean.txt', 'r')
lines = f.readlines()
for line in lines:
    line = line.strip()
    temp = line.split()
