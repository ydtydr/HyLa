...
with open('text/data_old/' + dataset + '.txt', 'r') as f:
    lines = f.readlines()
...
with open('text/corpus/' + dataset + '_labels.txt', 'w') as f:
    f.write('\n'.join(label_names))
...
doc_content_list = []
with open('text/corpus/' + dataset + '.clean.txt', 'r') as f:
    lines = f.readlines()
...
word_id_map = dict(zip(vocab, np.array(range(len(vocab)))))
vocab_size = len(vocab)

with open('text/corpus/' + dataset + '_vocab.txt', 'w') as f:
    vocab_str = '\n'.join(vocab)
    f.write(vocab_str)
...
def export_graph(graph, node_size0, node_size1, phase=""):
    ...
    if phase == "": path = "text/data/ind.{}.adj".format(dataset)
    else: path = "text/data/ind.{}.{}.adj".format(dataset, phase)
...
node_size = len(train_val_ids)+len(test_ids)
export_graph(B, len(train_val_ids)+len(test_ids), vocab_size, phase="B")

build_word_word_graph

# dump objects
f = open("text/data/ind.{}.{}.x".format(dataset, "train"), 'wb')
pkl.dump(train_ids, f)
f.close()

f = open("text/data/ind.{}.{}.y".format(dataset, "train"), 'wb')
pkl.dump(train_labels, f)
f.close()

f = open("text/data/ind.{}.{}.x".format(dataset, "val"), 'wb')
pkl.dump(val_ids, f)
f.close()

f = open("text/data/ind.{}.{}.y".format(dataset, "val"), 'wb')
pkl.dump(val_labels, f)
f.close()

f = open("text/data/ind.{}.{}.x".format(dataset, "test"), 'wb')
pkl.dump(test_ids, f)
f.close()

f = open("text/data/ind.{}.{}.y".format(dataset, "test"), 'wb')
